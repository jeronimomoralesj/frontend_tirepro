          ...
          {vehicle.union.length > 0 && (
            <div className="pt-2 border-t border-gray-50">
              ...
              <div className="flex flex-wrap gap-1.5 mt-1">
                {vehicle.union.map((p) => (
                  <span key={p} className="inline-flex items-center gap-1 text-xs font-semibold text-[#1E76B6] bg-[#1E76B6]/8 px-2 py-1 rounded-lg">
                    {p.toUpperCase()}
                    <button
                      type="button"
                      title="Desconectar"
                      onClick={(e) => { e.stopPropagation(); onRemoveUnion(p.toUpperCase()); }}
                      className="ml-0.5 p-0.5 rounded-full text-[#1E76B6]/50 hover:text-white hover:bg-red-500 transition-colors"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  </span>
  ...
  function renderCard(vehicle: Vehicle, indexInGroup: number, onRemoveLeft: (() => void) | null) {
    return (
      <VehicleCard
        ...
        onRemoveUnion={(placa) => setUnionToDelete({ sourceId: vehicle.id, targetPlaca: placa })}
              ...
              <section>
                ...
                <div className="space-y-6">
                  {connectedGroups.map((group, gi) => (
                    ...
                    >
                      {/* Group header — plates connected by line */}
                      <div className="px-4 pt-3 pb-2 flex items-center gap-2 flex-wrap">
                        <Link2 className="w-3.5 h-3.5 text-[#1E76B6]" />
                        {group.map((v, vi) => (
                          <React.Fragment key={v.id}>
                            {vi > 0 && <div className="w-6 h-px bg-[#1E76B6]/40" />}
                            <span className="text-xs font-black text-[#0A183A] tracking-wider">{v.placa.toUpperCase()}</span>
                          </React.Fragment>
                        ))}
                      </div>
