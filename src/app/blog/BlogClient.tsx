...
const BlogClient = ({ initialArticles = [] }: { initialArticles?: any[] }) => {
  ...
  const getCategories = () => {
    const uniqueCategories = [...new Set(articles.map((article: any) => article.category))]
    const categoryCounts = uniqueCategories.map(category => ({
      id: category,
      name: (category as string).charAt(0).toUpperCase() + (category as string).slice(1),
      count: articles.filter((article: any) => article.category === category).length
    }))
  ...
  const latestArticle = articles.length > 0
    ? [...articles].sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime())[0]
    : null

  const featuredArticles = filteredArticles.filter((a: any) => a.featured)
  const regularArticles = filteredArticles.filter((a: any) => !a.featured && a.id !== latestArticle?.id)
