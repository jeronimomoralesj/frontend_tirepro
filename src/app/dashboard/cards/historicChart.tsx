...
const translations = {
  es: {
    ...
    tooltipDate: "Fecha",
    tooltipValue: (label: string, value: number) =>
...
const HistoricChart: React.FC<HistoricChartProps> = ({ tires, language }) => {
  ...
  const variableDisplayName = useMemo(() => {
    switch (selectedVariable) {
      case "cpk":
        return "CPK";
      case "cpkProyectado":
        return "CPK Proyectado";
      case "profundidadInt":
        return "Profundidad Interior";
      case "profundidadCen":
        return "Profundidad Central";
      case "profundidadExt":
        return "Profundidad Exterior";
      default:
  ...
  const { inspectionDays, dailyAverages } = useMemo(() => {
    ...
    tires.forEach((tire) => {
      ...
      tire.inspecciones.forEach((inspection) => {
        ...
        if (!dateValueMap[dateKey]) dateValueMap[dateKey] = [];
  ...
  const chartData = useMemo(() => {
    const pointRadius = dailyAverages.map((_, idx) => {
      if (
        idx === 0 ||
        idx === Math.floor(inspectionDays.length / 2) ||
        idx === inspectionDays.length - 1
      )
        return 5;
      return 0;
    });
      ...
      tooltip: {
        ...
        callbacks: {
          title: (tooltipItems: TooltipItem<"line">[]) =>
            `${t.tooltipDate}: ${tooltipItems[0].label}`,
          label: (tooltipItem: TooltipItem<"line">) =>
            `${variableDisplayName}: ${Number(tooltipItem.raw).toFixed(2)}`,
        },
        borderColor: "#e2e8f0",
        borderWidth: 1,
        caretPadding: 8,
    ...
    <div className="bg-gradient-to-r from-[#173D68] to-[#1E76B6] text-white p-3 sm:p-4">
      ...
      <div className="relative">
        ...
        >
          ...
          <option value="cpkProyectado" className="text-gray-800 bg-white">
            {t.cpkProjected}
          </option>
          <option value="profundidadInt" className="text-gray-800 bg-white">
            {t.profundidadInt}
          </option>
          <option value="profundidadCen" className="text-gray-800 bg-white">
            {t.profundidadCen}
          </option>
          <option value="profundidadExt" className="text-gray-800 bg-white">
            {t.profundidadExt}
          </option>
        </select>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 p-3 sm:p-5">
        {/* Chart container — fixed height so it never overflows */}
        <div
          className="h-40 xs:h-48 sm:h-56 mb-3"
        ...
        <div className="border-t border-gray-100 pt-3 flex flex-col xs:flex-row justify-between items-start xs:items-center gap-2">
          <div className="flex items-center gap-2 bg-blue-50 px-3 py-1.5 rounded-lg">
            <Calendar size={13} className="text-blue-600 flex-shrink-0" />
            <span className="text-xs font-medium text-blue-700">
              {t.totalDays}:{" "}
              <span className="font-bold">{inspectionDays.length}</span>
            </span>
          </div>
          <div className="text-xs text-gray-500 italic">{t.footerNote}</div>
        </div>
      </div>
    </div>
  );
