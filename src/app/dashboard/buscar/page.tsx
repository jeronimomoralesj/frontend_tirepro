...
const BuscarPage: React.FC = () => {
      ...
      if (searchMode === "vehicle") {
        ...
        const tiresRes = await fetch(
          process.env.NEXT_PUBLIC_API_URL
            ...
            : `http://ec2-54-227-84-39.compute-1.amazonaws.com:6001/api/tires?companyId=${companyId}&placa=${encodeURIComponent(
                searchTerm.trim()
              )}`
        );
        if (!tiresRes.ok) {
  ...
  // Calculate average tread depth for a tire
  const calculateAvgTreadDepth = (tire: Tire) => {
    ...
    const avg = (latestInspection.profundidadInt + latestInspection.profundidadCen + latestInspection.profundidadExt) / 3;
    return avg.toFixed(1);
  };

  // Get status color based on tread depth
            ...
            <div className="p-6 bg-gray-50">
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                  ...
                  <p className="text-xl font-bold text-[#0A183A]">{vehicle.placa}</p>
                </div>
                {vehicle.tipovhc && (
                  <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                    <p className="text-sm text-gray-500 font-medium">Tipo</p>
                    <p className="text-xl font-bold text-[#0A183A]">{vehicle.tipovhc}</p>
                  </div>
                )}
                {vehicle.tipovhc && (
                  <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                    <p className="text-sm text-gray-500 font-medium">Carga</p>
                    <p className="text-xl font-bold text-[#0A183A]">{vehicle.carga}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6">
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {tires.map((tire) => {
                  ...
                  const statusColor = !isNaN(avgDepth) ? getTreadStatusColor(avgDepth) : "bg-gray-300";
                          ...
                          {tire.inspecciones && tire.inspecciones.length > 0 && (
                            <div className="flex items-center text-sm">
                              ...
                              <span className="text-gray-600">
                                Última inspección: {new Date(tire.inspecciones[tire.inspecciones.length - 1].fecha).toLocaleDateString()}
                              </span>
                            </div>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          <p className="font-medium">
                            ...
                            <span className="text-xs text-gray-500 ml-1">
                              ({new Date(selectedTire.costo[selectedTire.costo.length - 1].fecha).toLocaleDateString()})
                            </span>
                          </p>
                        </div>
                      )}
                ...
                <div>
                  ...
                  <div className="overflow-x-auto bg-white rounded-lg border border-gray-200">
                    <table className="w-full border-collapse">
                      ...
                      <tbody>
                        {selectedTire.inspecciones.map((insp, idx) => (
                          <tr 
                            key={idx} 
