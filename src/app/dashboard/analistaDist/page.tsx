...
import {
  ...
  Activity, Loader2, X, ChevronDown, Building2, Users,
} from "lucide-react";
...
const ClientSelector: React.FC<{
  ...
  const options = useMemo(() => {
    ...
    return all.filter(o => o.toLowerCase().includes(search.toLowerCase()));
  }, [companies, search]);
    ...
    <div className="relative">
      ...
      >
        ...
        <span className="max-w-[160px] truncate">
          {selected === "Todos" ? "Todos los clientes" : selected}
        </span>
      ...
      {open && (
        <>
          ...
          <div
            className="absolute right-0 mt-2 w-64 rounded-2xl overflow-hidden z-20"
            style={{
              background: "white",
              border: "1px solid rgba(52,140,203,0.15)",
          ...
          >
            ...
            <div className="p-2" style={{ borderBottom: "1px solid rgba(52,140,203,0.1)" }}>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-3 py-1.5 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-[#1E76B6]"
              ...
              {options.length === 0
                ...
                : options.map(name => (
                  <button
                    ...
                    style={{
                      fontWeight: selected === name ? 800 : 500,
                      color: selected === name ? "#1E76B6" : "#0A183A",
                    }}
                  >
                    {name === "Todos"
                      ? <span className="flex items-center gap-2"><Users className="w-3.5 h-3.5 opacity-60" />Todos los clientes</span>
                      : name
                    }
                  </button>
                ))
        ...
        <div className="mb-4">
          ...
          <div className="space-y-1.5">
            {tire.recomendaciones.map((rec, i) => (
              ...
              >
                <span className="w-1.5 h-1.5 rounded-full mt-1 flex-shrink-0" style={{ background: "#1E76B6" }} />
  ...
  useEffect(() => {
    authFetch(`${API_BASE}/companies/me/clients`)
      ...
      .then((data: any[]) => {
        ...
        setCompanies(data.map((a: any) => ({
          id:   a.company?.id   ?? a.id,
          name: a.company?.name ?? a.name,
        })));
      })
      .catch(console.error);
  }, []);
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8 space-y-4 sm:space-y-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/60 mt-0.5">
                  Diagnóstico predictivo por vehículo · Vista Distribuidor
                </p>
              </div>
            </div>

            {/* Client selector — only rendered when there are clients */}
            {companies.length > 0 && (
              <ClientSelector
                companies={companies}
                selected={selectedClient}
                onSelect={name => { setSelectedClient(name); setAnalysis(null); setPlaca(""); setSearchError(""); }}
              />
            )}
          </div>
          ...
          {selectedClient !== "Todos" && (
            <div className="flex items-center gap-2 mt-3">
              ...
              >
                ...
                {selectedClient}
                <button
                  onClick={() => { setSelectedClient("Todos"); setAnalysis(null); setPlaca(""); setSearchError(""); }}
                  className="ml-1 hover:text-red-300 transition-colors opacity-70 hover:opacity-100"
                >
                  <X className="w-3 h-3" />
                </button>
              </span>
        ...
        <Card className="p-4 sm:p-5">
          <CardTitle
            ...
            sub={
              selectedClient !== "Todos"
                ? `Diagnóstico para cliente: ${selectedClient}`
                : "Diagnóstico detallado de todas las llantas de un vehículo"
            }
          />
            ...
            >
              ...
              <div className="flex gap-2 flex-wrap">
                {(() => {
                  ...
                  return (
                    <>
                      {critical > 0 && <span className="px-2 py-0.5 rounded-full text-[11px] font-black" style={{ background: "rgba(220,38,38,0.1)", color: "#DC2626" }}>{critical} crítico{critical !== 1 ? "s" : ""}</span>}
                      {warn     > 0 && <span className="px-2 py-0.5 rounded-full text-[11px] font-black" style={{ background: "rgba(217,119,6,0.1)",  color: "#D97706" }}>{warn} precaución</span>}
                      {ok       > 0 && <span className="px-2 py-0.5 rounded-full text-[11px] font-black" style={{ background: "rgba(34,197,94,0.1)",   color: "#16a34a" }}>{ok} OK</span>}
                    </>
                  );
                })()}
              </div>
        ...
        {!analysis && !searchLoading && (
          <Card className="p-10 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-xs text-gray-400 max-w-xs">
              El sistema analizará todas las llantas del vehículo y generará recomendaciones automáticas
              {selectedClient !== "Todos" ? ` para ${selectedClient}` : ""}.
            </p>
          </Card>
        )}
      </div>
