...
function TireTooltip({ tire, anchor }: { tire: Tire; anchor: HTMLElement | null }) {
  ...
  // Portal-rendered so the tooltip is never clipped by the Inventario
  // scroll container (`overflow-y-auto max-h-64`) or any other ancestor.
  // Position tracks the anchor via getBoundingClientRect; we recompute on
  // window scroll/resize and on a low-frequency RAF loop so drag previews
  // and other DOM shifts stay in sync.
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const placement: "top" | "bottom" = spaceAbove >= TIP_HEIGHT + GAP ? "top" : "bottom";
      const top = placement === "top" ? r.top - GAP - TIP_HEIGHT : r.bottom + GAP;
...
function VehicleVisualization({ tires, onTireDrop, fixedLayout, onLayoutChange, configuracion, onEditStructure }: {
  ...
  const computedLayout = React.useMemo(() => {
    ...
    if (base.length === 0) {
      ...
      // remainder" algorithm. See axleLayoutFallback.ts for the rule.
...
export default function PosicionPage() {
  ...
  const handleDropToBucket = useCallback(async (tireId: string, bucketId: string, bucketName: string) => {
    ...
    // Tile was hidden while on the vehicle — unhide so it reappears.
    ...
    try {
      ...
      if (external.length > 0) {
        const aRes = await fetch(`${API_BASE}/tires/assign-vehicle`, {
          ...
          // Send vehicleId (unique) — placa can match multiple rows after
          // merquepro-v2 (orphan + active sharing the same plate).
