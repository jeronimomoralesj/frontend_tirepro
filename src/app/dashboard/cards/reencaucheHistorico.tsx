...
const translations = {
  es: {
    ...
    month: "Mes",
    retreadsQuantity: "Reencauches",   // shortened for mobile
    cumulativeTotal: "Total Acum.",    // shortened for mobile
    tooltipIndividual:
      "Esta tabla muestra cuántos reencauches has tenido a lo largo de los últimos meses.",
    tooltipCumulative:
      " La vista acumulada acumula el total hasta cada mes.",
...
const ReencaucheHistorico: React.FC<ReencaucheHistoricoProps> = ({
  ...
  const countsToShow =
    mode === "individual" ? individualCounts : acumuladoCounts;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden w-full flex flex-col">
      ...
      <div className="bg-gradient-to-r from-[#173D68] to-[#1E76B6] text-white p-3 sm:p-4">
        <div className="flex items-center justify-between mb-3 gap-2">
          <div className="flex items-center gap-2 min-w-0">
            <History size={18} className="text-white/90 flex-shrink-0" />
          ...
          <div className="relative flex-shrink-0">
            ...
            {showTooltip && (
              <div className="absolute z-20 right-0 top-full mt-2 w-56 sm:w-64 bg-gray-900/95 backdrop-blur-sm text-white text-xs p-3 rounded-lg shadow-xl border border-white/10">
                <p className="leading-relaxed">
                  {t.tooltipIndividual}
                  {mode === "acumulado" && t.tooltipCumulative}
                </p>
                <div className="absolute -top-1 right-4 w-2 h-2 bg-gray-900/95 transform rotate-45 border-l border-t border-white/10" />
              </div>
            )}
          </div>
        </div>
      ...
      {/* Table — scrollable vertically on small screens */}
      <div className="flex-1 p-3 sm:p-5 overflow-x-auto">
        <table className="w-full min-w-[240px]">
          <thead>
            <tr className="border-b-2 border-gray-200">
              <th className="pb-2 sm:pb-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                {t.month}
              </th>
              <th className="pb-2 sm:pb-3 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">
                {mode === "individual"
                  ? t.retreadsQuantity
                  : t.cumulativeTotal}
              </th>
          ...
          <tbody>
            {months.map((m, idx) => (
              ...
              >
                <td className="py-2 sm:py-3 text-xs sm:text-sm text-gray-700 font-medium">
                  {m}
                </td>
                <td className="py-2 sm:py-3 text-right">
                  <span
                    className={`inline-flex items-center justify-center min-w-[2rem] px-2 py-0.5 rounded-lg text-xs sm:text-sm font-bold ${
                      countsToShow[m] > 0
      ...
      {/* Footer */}
      <div className="border-t border-gray-100 px-3 py-2.5 sm:px-5 sm:py-3 bg-gray-50">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 bg-blue-50 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg">
            <History size={13} className="text-blue-600 flex-shrink-0" />
            <span className="text-xs font-medium text-blue-700">
              {t.totalRetreads}:{" "}
              <span className="font-bold">{totalRetreads}</span>
            </span>
          </div>
          <div className="text-xs text-gray-500 italic flex-shrink-0">
